    ...
    return(
        <div className={styles.container} >
            ...
            <div className={styles.testDetails}>
                {assessment.status === 'Completed' ? <button className={calculateResult(assessment.score, assessment.quiz.length) === 'Pass' ? `${styles.status} ${styles.success}` : `${styles.status} ${styles.warning}`}>{calculateResult(assessment.score, assessment.quiz.length)}</button> : 
